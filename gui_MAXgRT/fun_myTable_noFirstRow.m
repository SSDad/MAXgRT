function [hEdit] = fun_myTable_noFirstRow(parent, RowRatio, columnRatio, txt, FS, bgc)

junk = 1+sum(columnRatio);
wCell = [1 columnRatio]/junk;
nC = length(columnRatio);
junk = cumsum(wCell);
xP = [0 junk(1:end-1)];

junk = sum(RowRatio);
hCell = RowRatio/junk;
nR = length(RowRatio);

% all cells
yP = 1;
for iR = 1:nR
    yP = yP-hCell(iR);
    for iC = 1:nC+1
        hET(iR, iC) = uipanel('parent', parent,...
                                'Unit', 'Normalized',...
                                'Position', [xP(iC) yP wCell(iC) hCell(iR)],...
                                'Units',                     'normalized', ...
                                'visible',                      'on', ...
                                'ForegroundColor',       'k',...
                                'BackgroundColor',       bgc, ...
                                'HighlightColor',          'w',...
                                'ShadowColor',            'k');
    end
end

% first column
for iR = 1:nR
    hText.FirsColumn(iR) = uicontrol('parent', hET(iR, 1), ...
                                'Style', 'text',...
                                'String', txt.FirstColumn{iR},...
                                'Unit', 'Normalized',...
                                'Position', [0 0 1 1], ...
                                'FontSize', FS, ...
                                'FontWeight', 'bold', ...
                                'BackgroundColor', bgc,...
                                'ForegroundColor', 'w',...
                                'Visible', 'on');
end

% data
for iC = 1:nC
    for iR = 1:nR
        hEdit(iR, iC) = uicontrol('parent', hET(iR, iC+1), ...
                                'Style', 'edit',...
                                'Unit', 'Normalized',...
                                'Position', [0 0 1 1],...
                                'FontSize', FS, ...
                                'FontWeight', 'bold', ...
                                'BackgroundColor', bgc,...
                                'ForegroundColor', 'g',...
                                'String', txt.DataStr{iR, iC}, ...
                                'Visible', 'on');

    end
end